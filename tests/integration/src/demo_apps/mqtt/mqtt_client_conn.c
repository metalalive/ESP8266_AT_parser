...
int mqttClientInit(mqttCtx_t **mctx, int cmd_timeout_ms, word32 tx_buf_len, word32 rx_buf_len) {
    ...
    c->retain_avail = 1;
    *mctx = c;
    // TODO: create semaphores from packet send/receive operations in
    // multithreading case.
...
int mqttSendConnect(mqttCtx_t *mctx) {
    ...
    // the return value must be equal to length of Tx buffer, otherwise
    // something in the underlying system must be wrong.
    if (status != MQTT_RETURN_SUCCESS) {
        return status;
    }
...
int mqttSendDisconnect(mqttCtx_t *mctx) {
    ...
    tx_buf_len = mctx->tx_buf_len;
    pkt_total_len =
        mqttEncodePktDisconn(tx_buf, tx_buf_len, (mqttPktDisconn_t *)&mctx->send_pkt.disconn);
    if (pkt_total_len <= 0) {
        return MQTT_RETURN_ERROR_MALFORMED_DATA;
    }
...
int mqttSendSubscribe(mqttCtx_t *mctx) {
    ...
    status = mqttClientWaitPkt(mctx, MQTT_PACKET_TYPE_SUBACK, subs->packet_id, (void *)suback);
    return status;
...
int mqttSendUnsubscribe(mqttCtx_t *mctx) {
    ...
    status =
        mqttClientWaitPkt(mctx, MQTT_PACKET_TYPE_UNSUBACK, unsubs->packet_id, (void *)unsuback);
    return status;

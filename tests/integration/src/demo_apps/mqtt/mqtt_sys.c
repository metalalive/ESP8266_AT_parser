...
// [Note]
// The low-level implementation should depend on what knid of underlying
// hardware / operating system applied to your application, this file only shows
// API integration with our ESP AT parser, used with ESP8266 wifi device. For
// Linux / Windows user, you will need to implement your own socket application.

// In some use cases, users might call this read function multiple times only
// for fetching few bytes of packet data. so we need to reserve the packet data
// that hasn't been completed reading (from user applications)

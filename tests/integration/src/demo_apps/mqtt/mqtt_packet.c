...
word32 mqttEncodeProps(byte *buf, mqttProp_t *props) {
    ...
    for (curr_prop = props; curr_prop != NULL; curr_prop = curr_prop->next) {
        ...
        case MQTT_DATA_TYPE_STRING:
            len =
                mqttEncodeStr(buf, (const char *)curr_prop->body.str.data, curr_prop->body.str.len);
            break;
        case MQTT_DATA_TYPE_STRING_PAIR:
            len = mqttEncodeStr(
                buf, (const char *)curr_prop->body.strpair[0].data, curr_prop->body.strpair[0].len
            );
            if (buf != NULL) {
                buf += len;
            }
            total_len += len;
            len = mqttEncodeStr(
                buf, (const char *)curr_prop->body.strpair[1].data, curr_prop->body.strpair[1].len
            );
            break;
        default:
            len = 0;
...
word32 mqttDecodeProps(byte *buf, mqttProp_t **props, word32 props_len) {
    ...
    while (props_len > 0) {
        ...
        case MQTT_DATA_TYPE_STRING:
            len = mqttDecodeStr(
                buf, (const char **)&curr_prop->body.str.data, &curr_prop->body.str.len
            );
            break;
        case MQTT_DATA_TYPE_STRING_PAIR:
            len = mqttDecodeStr(
                &buf[0], (const char **)&curr_prop->body.strpair[0].data,
                &curr_prop->body.strpair[0].len
            );
            len += mqttDecodeStr(
                &buf[len], (const char **)&curr_prop->body.strpair[1].data,
                &curr_prop->body.strpair[1].len
            );
            break;
        default:
            len = 0;
...
static word32 mqttEncodeFxHeader(
    byte *tx_buf, word32 tx_buf_len, word32 remain_len, mqttCtrlPktType cmdtype, byte retain,
    byte qos, byte duplicate
) {
    word32           len = 0;
...
static word32 mqttDecodeFxHeader(
    byte *rx_buf, word32 rx_buf_len, word32 *remain_len, mqttCtrlPktType cmdtype, byte *retain,
    byte *qos, byte *duplicate
) {
    ...
    if (retain != NULL) {
        *retain = (header->type_flgs >> 0) & 0x1;
    }
    if (qos != NULL) {
        *qos = (header->type_flgs >> 1) & 0x3;
    }
    if (duplicate != NULL) {
        *duplicate = (header->type_flgs >> 3) & 0x1;
    }
    len += 1;
    len += mqttDecodeVarBytes(&header->remain_len[0], &_remain_len);
    if (remain_len != NULL) {
        *remain_len = _remain_len;
    }
    return len;
...
word32 mqttEncodePktConnect(byte *tx_buf, word32 tx_buf_len, struct __mqttConn *conn) {
    ...
    word32               remain_len = 0;
    word32               props_len = 0;
    mqttPktHeadConnect_t var_head = {
        {0, MQTT_CONN_PROTOCOL_NAME_LEN}, {'M', 'Q', 'T', 'T'}, 0, 0, 0
    };
    byte *curr_buf_pos;
    ...
    // size of variable header in CONNECT packet, should be 10 bytes in MQTT
    // v5.0
    ...
    fx_head_len =
        mqttEncodeFxHeader(tx_buf, tx_buf_len, remain_len, MQTT_PACKET_TYPE_CONNECT, 0, 0, 0);
    ...
    curr_buf_pos +=
        mqttEncodeStr(curr_buf_pos, (const char *)conn->client_id.data, conn->client_id.len);
    // TODO: implement will property, will topic, and will payload
    if (conn->username.data != NULL) {
        curr_buf_pos +=
            mqttEncodeStr(curr_buf_pos, (const char *)conn->username.data, conn->username.len);
    } else {
        // [Note]
        // A server may allow a client to provide empty clientID (has length of
        // zero byte), server must assign unique ID to such CONNECT packet (with
        // zero-byte clientID), and then return CONNACK packet with the property
        // "Assigned Client Identifier" back to client .
        curr_buf_pos += mqttEncodeWord16(curr_buf_pos, (word16)0);
    }
    if (conn->password.data != NULL) {
        curr_buf_pos +=
            mqttEncodeStr(curr_buf_pos, (const char *)conn->password.data, conn->password.len);
    }
...
word32 mqttDecodePktConnack(byte *rx_buf, word32 rx_buf_len, mqttPktHeadConnack_t *connack) {
    ...
    byte  *curr_buf_pos;
    byte  *end_of_buf;
    fx_head_len = mqttDecodeFxHeader(
        rx_buf, rx_buf_len, &remain_len, MQTT_PACKET_TYPE_CONNACK, NULL, NULL, NULL
    );
...
word32 mqttEncodePktDisconn(byte *tx_buf, word32 tx_buf_len, mqttPktDisconn_t *disconn) {
    ...
    // if reason code is 0x0 (normal disconnection), then there's no need to put
    // reason code and extra properties into DISCONNECT packet, therefore hte
    ...
    // build fixed header of CONNECT packet
    fx_head_len =
        mqttEncodeFxHeader(tx_buf, tx_buf_len, remain_len, MQTT_PACKET_TYPE_DISCONNECT, 0, 0, 0);
    if ((reason_code != MQTT_REASON_NORMAL_DISCONNECTION) || (disconn->props != NULL)) {
        curr_buf_pos = &tx_buf[fx_head_len];
...
word32 mqttEncodePktPublish(byte *tx_buf, word32 tx_buf_len, struct __mqttMsg *msg) {
    ...
    fx_head_len = mqttEncodeFxHeader(
        tx_buf, tx_buf_len, (var_head_len + payload_len), MQTT_PACKET_TYPE_PUBLISH, msg->retain,
    ...
    // [NOTE]
    //  *  Copy the beginning part of application specific data to the Tx buffer
    //  at here
    //     , the rest of the application data will be copied at the level of
    //     mqttSendPublish() .
    //  *  It's acceptable that the size of application data is greater than
    //  transfer
    //     buffer (the Tx buffer here), in such case the MQTT client must break
    //     the huge buffer of application data into several pieces, in order to
    //     fit each piece into network packet.
...
word32 mqttDecodePktPublish(byte *rx_buf, word32 rx_buf_len, struct __mqttMsg *msg) {
    ...
    fx_head_len = mqttDecodeFxHeader(
        rx_buf, rx_buf_len, &payload_len, MQTT_PACKET_TYPE_PUBLISH, &msg->retain, &msg->qos,
        &msg->duplicate
    );
    curr_buf_pos = &rx_buf[fx_head_len];
    // variable header : topic filter
    ...
    // variable header : check QoS & see if we have packet ID field in the
    ...
    // payload, if Rx buffer is insufficient to store entire application
    // message, then we only store the beginning part (and lose part of the
    // application message)
...
word32 mqttEncodePktSubscribe(byte *tx_buf, word32 tx_buf_len, mqttPktSubs_t *subs) {
    ...
    fx_head_len = mqttEncodeFxHeader(
        tx_buf, tx_buf_len, remain_len, MQTT_PACKET_TYPE_SUBSCRIBE, 0, MQTT_QOS_1, 0
    );
    curr_buf_pos = &tx_buf[fx_head_len];
    ...
    for (idx = 0; idx < subs->topic_cnt; idx++) {
        curr_topic = &subs->topics[idx];
        curr_buf_pos += mqttEncodeStr(
            curr_buf_pos, (const char *)curr_topic->filter.data, curr_topic->filter.len
        );
        *curr_buf_pos = (byte)curr_topic->qos; // TODO: implement all fields of
...
word32 mqttEncodePktUnsubscribe(byte *tx_buf, word32 tx_buf_len, mqttPktUnsubs_t *unsubs) {
    ...
    fx_head_len = mqttEncodeFxHeader(
        tx_buf, tx_buf_len, remain_len, MQTT_PACKET_TYPE_UNSUBSCRIBE, 0, MQTT_QOS_1, 0
    );
    curr_buf_pos = &tx_buf[fx_head_len];
    // variable header, packet ID, and optional properties
...
word32 mqttEncodePktPubResp(
    byte *tx_buf, word32 tx_buf_len, mqttPktPubResp_t *resp, mqttCtrlPktType cmdtype
) {
...
word32 mqttDecodePktPubResp(
    byte *rx_buf, word32 rx_buf_len, mqttPktPubResp_t *resp, mqttCtrlPktType cmdtype
) {
...
word32 mqttDecodePktSuback(byte *rx_buf, word32 rx_buf_len, mqttPktSuback_t *suback) {
    ...
    byte  *curr_buf_pos;
    fx_head_len = mqttDecodeFxHeader(
        rx_buf, rx_buf_len, &remain_len, MQTT_PACKET_TYPE_SUBACK, NULL, NULL, NULL
    );
    curr_buf_pos = &rx_buf[fx_head_len];
    ...
    // the SUBACK payload contains a list of return codes that indicate whether
    // the topic filters are subscribed successfully on the borker side.
...
word32 mqttDecodePktUnsuback(byte *rx_buf, word32 rx_buf_len, mqttPktUnsuback_t *unsuback) {
    ...
    byte  *curr_buf_pos;
    fx_head_len = mqttDecodeFxHeader(
        rx_buf, rx_buf_len, &remain_len, MQTT_PACKET_TYPE_UNSUBACK, NULL, NULL, NULL
    );
    curr_buf_pos = &rx_buf[fx_head_len];
    ...
    // the UNSUBACK payload contains a list of return codes that indicate
    // whether the topic filters are unsubscribed successfully on the borker
    // side.
...
int mqttPktRead(struct __mqttCtx *mctx, byte *buf, word32 buf_max_len, word32 *copied_len) {
    ...
    if (tmp == 0x0) {
        return MQTT_RETURN_ERROR_TIMEOUT;
    }
    buf += tmp;
    header_len = tmp;
    // read from the 2nd byte, determined remain length encoded in variable
    // bytes.
    ...
    if (idx == MQTT_PKT_MAX_BYTES_REMAIN_LEN) {
        return MQTT_RETURN_ERROR_MALFORMED_DATA; // Error (Malformed Remaining
                                                 // Len)
    }
    ...
    *copied_len = header_len;
    ...
    if (buf_max_len < (remain_len + header_len)) {
        // current Rx buffer cannot hold entire packet, chances are that
        // received application message exceeds the limit of Rx buffer ,so we
...
int mqttDecodePkt(
    struct __mqttCtx *mctx, byte *buf, word32 buf_len, void *p_decode, word16 *recv_pkt_id
) {
    const mqttPktFxHead_t *header = (mqttPktFxHead_t *)buf;

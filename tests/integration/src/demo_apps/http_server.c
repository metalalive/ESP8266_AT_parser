...
static const char resp_msg_header2[] =
    ""
    " " ESP_CHR_CR ESP_CHR_LF ESP_CHR_CR ESP_CHR_LF "<html><head></head>"
    "<body>"
...
static const char resp_msg_footer1[] =
    ...
    "</p>"
    "</form>"
    "<p>try putting random number of words in the text field above then click "
    "submit button</p>"
...
static const char resp_msg_footer2[] =
    ""
    "</b></p>"
    "<p>POST method checked. <a href='/webfmpg'>Go back to web-form "
    "page</a></p>"
    "<br><br>"
    "</body></html>" ESP_CHR_CR ESP_CHR_LF;
...
static void vESPtestHttpSimpleApp(espPbuf_t *pktbuf, httpTestMsg_t *http_msg_p) {
    ...
    ESP_MEMSET(
        resp_msg_p, 0x00, sizeof(resp_msg_header2) + sizeof(resp_msg_footer1) + TEST_MAX_CHARS_URI
    );
    ...
    num_bytes_uri = uESPparseStrUntilToken(
        (char *)&http_msg_p->uri[0], (const char *)content_p, TEST_MAX_CHARS_URI, ' '
    );
    ...
    num_bytes_html_entity +=
        (http_msg_p->method == HTTP_METHOD_POST ? sizeof(resp_msg_footer2)
                                                : sizeof(resp_msg_footer1));
    num_str_cvt =
        uiESPcvtNumToStr(&content_length_str[0], num_bytes_html_entity, ESP_DIGIT_BASE_DECIMAL);
...
void vCreateAllTestTasks(void) {
    ...
    http_msg.resp_msg = (uint8_t *)ESP_MALLOC(
        sizeof(resp_msg_header1) + sizeof(resp_msg_header2) + sizeof(resp_msg_footer1) +
        TEST_MAX_CHARS_URI
    );

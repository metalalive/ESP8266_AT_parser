...
espRes_t eESPtestConnAP(uint8_t waitUntilConnected) {
    ...
    do {
        ...
        response = eESPstaListAP(
            preferredAP.ssid, (uint16_t)preferredAP.ssid_len, &foundAPs[0], ESP_ARRAYSIZE(foundAPs),
            &num_ap_found, NULL, NULL, ESP_AT_CMD_BLOCKING
        );
            ...
            for (idx = 0; idx < num_ap_found; idx++) {
                ...
                if (!strcmp(preferredAP.ssid, foundAPs[idx].ssid)) {
                    ...
                    response = eESPstaJoin(
                        preferredAP.ssid, preferredAP.ssid_len, preferredAP.passwd,
                        preferredAP.passwd_len, NULL, ESP_SETVALUE_NOT_SAVE, NULL, NULL,
                        ESP_AT_CMD_BLOCKING
                    );
                    if (response == espOK) {
                        espIp_t  sta_ip, ap_ip;
                        espMac_t sta_mac, ap_mac;
                        // TODO: figure out why my ESP-01s never responds when
                        // we ask for IP address
                        eESPgetLocalIPmac(
                            &sta_ip, &sta_mac, &ap_ip, &ap_mac, NULL, NULL, ESP_AT_CMD_BLOCKING
                        );
                        waitUntilConnected = 0x0;
                        //// printf("[INFO] Connected to %s network!\r\n",
                        /// foundAPs[idx].ssid); / printf("[INFO] Station IP
                        /// address: %d.%d.%d.%d\r\n", /    (int)ip.ip[0],

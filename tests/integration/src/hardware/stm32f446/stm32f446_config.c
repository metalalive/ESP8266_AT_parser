...
// in each system port, DMA/UART ISR should specify starting offset & number of characters to
// copy to ESP AT software.
...
void HAL_UART_MspInit(UART_HandleTypeDef *huart) {
    ...
    if (huart->Instance == USART1) {
        ...
        HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);
               ...
               USART2) { // TODO: figure out why USART2 of my STM32 board doesn't work
...
void USART3_IRQHandler(void) {
    ...
    if (__HAL_UART_GET_FLAG(&haluart3, UART_FLAG_IDLE)) {
        ...
        // TODO: following code is likely to cause problem when DMA TC event triggers CPU, CPU
        // executes following macro code,
        //       , then DMA Rx is reinitialized next time. that will cause Rx fails to receive
        //       any character.
        ...
        eESPappendRecvRespISR(
            (haluart3.pRxBuffPtr + dma_buf_cpy_offset_curr), dma_buf_num_char_copied
        );
        dma_buf_cpy_offset_curr = dma_buf_cpy_offset_next;
    }
} // end of USART3_IRQHandler

void DMA1_Stream1_IRQHandler(void) { HAL_DMA_IRQHandler(&haldma_usart3_rx); }

// [TODO]
// calculate which half of the Rx buffer should be copied to ESP core function at here, (since
// the way to generate DMA/UART interrupt for HT/TC event depends on hardware platform.
// executed by DMA Half Transmission (HT) event interrupt
...
espRes_t eESPlowLvlRecvStartFn(void) {
    ...
    dma_buf_cpy_offset_curr = 0;
    status_chk =
        HAL_UART_Receive_DMA(&haluart3, (uint8_t *)&recv_data_buf[0], ESP_RESP_RECV_BUF_SIZE);
    switch (status_chk) {
    case HAL_OK:
        response = espOK;
        break;

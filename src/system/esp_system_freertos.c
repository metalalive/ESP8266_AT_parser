...
PRIVILEGED_FUNCTION espRes_t eESPsysThreadCreate(
    ...
    if (isPrivileged != 0x0) {
        ...
        tskparams.xRegions[0].ulLengthInBytes =
            (UBaseType_t)__unpriv_data_end__ - (UBaseType_t)__unpriv_data_start__;
        tskparams.xRegions[0].ulParameters =
            portMPU_REGION_READ_WRITE | MPU_RASR_S_Msk | MPU_RASR_C_Msk | MPU_RASR_B_Msk;
    }
...
PRIVILEGED_FUNCTION espRes_t eESPsysThreadDelete(espSysThread_t *t) {
    ...
    // in FreeRTOS implementation, if a task deletes itself,  vTaskDelete will call task
    // yielding function and switch to other tasks, therefore the deleted task won't get here.
    return espOK;
}
...
espRes_t eESPsysProtect(void) { return eESPsysMtxLock(&internal_mutex); }

espRes_t eESPsysUnprotect(void) { return eESPsysMtxUnlock(&internal_mutex); }
...
void vESPsysDelay(const uint32_t ms) { vTaskDelay(pdMS_TO_TICKS(ms)); }

#if (configCHECK_FOR_STACK_OVERFLOW > 0)
void vApplicationStackOverflowHook(espSysThread_t xTask, char *pcTaskName) {
    // this is software approach to let developers know stack overflow happened on last running
    // task , this hook function is only useful for debugging purpose, not for preventing it
    // from happening. For more instant detection on stack overflow, you need to enable
    // hardware MPU or MMU then it will signal memory fault (or exception) when stack overflow
    // is happening on a task.
    configASSERT(0);

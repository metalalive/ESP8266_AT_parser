...
static espRes_t eESPdeflEvtCallback(espEvt_t *evt) { return espOK; } // end of eESPdeflEvtCallback
...
espRes_t eESPinit(espEvtCbFn cb) {
    ...
    if (init_fail == 0) {
        // create message boxes for buffering AT-command request / response between
        // ESP device
    ...
    if (init_fail == 0) {
        ...
        response = eESPsysThreadCreate(
            &espGlobal.thread_cmd_req, "ATcmdReq", vESPthreadATreqHandler, NULL,
            ESP_SYS_THREAD_STACK_SIZE, ESP_SYS_THREAD_PRIO, isThreadPrivileged
        );
        response = eESPsysThreadCreate(
            &espGlobal.thread_cmd_resp, "ATcmdResp", vESPthreadATrespHandler, NULL,
            ESP_SYS_THREAD_STACK_SIZE, ESP_SYS_THREAD_PRIO, isThreadPrivileged
        );

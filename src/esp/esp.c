...
void vESPmsgDelete(espMsg_t *msg) {
    // TODO: figure out why exception/fault happen when freeing the allocated
    // memory.
    if (msg == NULL) {
        return;
    }
...
static espRes_t eESPinitInterTaskCommu(espGlbl_t *glb) {
    // creating semaphores accessed between handling threads & interrupt service
    // routine.
    glb->sem_th_sync = xESPsysSemCreate();
    if (glb->sem_th_sync == NULL) {
        return espERR;
    }
    // create message boxes for buffering AT-command request or
    // response between ESP device
    glb->mbox_cmd_req = xESPsysMboxCreate(ESP_CFG_AT_CMD_REQ_MBOX_SIZE);
    if (glb->mbox_cmd_req == NULL) {
        return espERR;
    }
    glb->mbox_cmd_resp = xESPsysMboxCreate(ESP_CFG_AT_CMD_RESP_MBOX_SIZE);
    if (glb->mbox_cmd_resp == NULL) {
        return espERR;
    }
    return espOK;
}

// initialize ESP AT library, this function must be called from operating
// system thread. this function creates threads that work together to handler
// AT command requests generated by API functions call, also a couple of data
// structure for internal use like mutex, semaphore (for thread
// synchronization), and message boxes (for communication between these
// threads).
espRes_t eESPinit(espEvtCbFn cb) {
    espRes_t response = espOK;
    uint8_t  init_fail = 0, isThreadPrivileged = 0x1;
    // clear initialized flag
    espGlobal.status.flg.initialized = 0;
    espGlobal.status.flg.dev_present = 0;
    // clear internal system lock
    espGlobal.locked_cnt = 0;
    espGlobal.ops.recv_resp_proc = eESPprocessPieceRecvResp;
    ...
    if (init_fail == 0) {
        response = eESPinitInterTaskCommu(&espGlobal);
        if (response != espOK) {
            init_fail++;
        }
    }
    if (init_fail == 0) {
        ...
        //
        // A trick can be used at here (the detail depends on your underlying
        // system implementation) :
        //     the thread calling this ESP init function can take higher priority
        //     over the 2 threads below, once the 2 threads are created, that
        //     makes them (the 2 threads below) NOT preempt current running thread,
        //     so current running thread goes on to complete the initialization
        //     code.
        espGlobal.thread_cmd_req = NULL;
        espGlobal.thread_cmd_resp = NULL;
        response = eESPsysThreadCreate(
            &espGlobal.thread_cmd_req, "ATcmdReq", vESPthreadATreqHandler, (void *const)&espGlobal,
            ESP_SYS_THREAD_STACK_SIZE, ESP_SYS_THREAD_PRIO, isThreadPrivileged
        );
        response = eESPsysThreadCreate(
            &espGlobal.thread_cmd_resp, "ATcmdResp", vESPthreadATrespHandler,
            (void *const)&espGlobal, ESP_SYS_THREAD_STACK_SIZE, ESP_SYS_THREAD_PRIO,
            isThreadPrivileged
        );
        if ((response != espOK) || (espGlobal.thread_cmd_req == NULL) ||
            (espGlobal.thread_cmd_resp == NULL)) {
            init_fail++;
        }
    }
...
espRes_t eESPcoreLock(espGlbl_t *glb) {
...
espRes_t eESPcoreUnlock(espGlbl_t *glb) {
...
espRes_t eESPsendReqToMbox(espMsg_t *msg, espRes_t (*initATcmdFn)(espMsg_t *, espGlbl_t *)) {
    ...
    // check any situation that could make this function fail to send request to
    // message box.
    eESPcoreLock(&espGlobal);
    ...
    if (msg->is_blocking == ESP_AT_CMD_BLOCKING) {
        // if we transfer the AT command request in blocking mode, then a semaphore
        // is required for synchronizing the transmission between request handling
        // thread and respense handling thread, respense handling thread will give
        // this semaphore to this thread when it receives complete response (from
        // the ESP device) .
    ...
    // succeed to put request to message box queue, it will be processed in
    // another thread
    msg->res = espINPROG;
    // From here on, the request handling thread should receive request from the
    // message box.
    if (msg->is_blocking == ESP_AT_CMD_BLOCKING) {
        // wait for respense handling thread receiving the entire response message
        // of current AT command request , timeout would happen if following
        // semaphore wait function returns espTIMEOUT.
...
void vESPapiRunEvtCallbacks(espMsg_t *msg, espEvt_t *evtp) {
...
espRes_t eESPflushMsgBox(espSysMbox_t mb, const espMemFreeStructCbFn cb) {
    ...
    while (1) {
        ...
        if (response == espOK) {
            if (cb != NULL) {
                cb(itemp);
            } // user application must take responsibility to free up all space
              // allocated
            else {
                ESP_MEMFREE(itemp);
            }

...
void vESPthreadATreqHandler(void *const arg) {
    ...
    for (;;) {
        ...
        // for few API funcion calls, we run the event callback function to
        // notify user application for important events.
        vESPapiRunEvtCallbacks(msg);
        if (is_blocking == ESP_AT_CMD_BLOCKING) {
            // for blocking AT-command request, this thread releases the
            // semaphore of the message, so the blocked API caller thread can be
            // resumed from semaphore wait function.
            eESPsysSemRelease(msg->sem);
        } else {
            // run callback function for the API function (regardless of its
            // response), this is useful when users made non-blocking API call
            // and attempts to see the result of AT command.
            if (msg->api_cb != NULL) {
                msg->api_cb(msg->res, msg->api_cb_arg);
            }
            // for non-blocking AT-command request, there is no semaphore
            // between this thread & the API caller, we can simply free the
            // space allocated to the message structure
            vESPmsgDelete(msg);
        }
        espGlobal.msg = NULL;
...
void vESPthreadATrespHandler(void *const arg) {
    ...
    for (;;) {
        ...
        // complete parsing the received string, now we can free the allocated
        // space
        ESP_MEMFREE(recv_buf);
        recv_buf = NULL;
        // wait until we receive entire response string of the AT command / IPD
        // data.
        if (isEndOfATresp != 0) {
            eESPcoreUnlock();
            eESPsysSemRelease(espGlobal.sem_th_sync);
            // yield this thread voluntarily, let next AT-command request come
            // in & be produced (if exists)
            eESPsysThreadYield();
            eESPcoreLock();
            isEndOfATresp = 0;
        }

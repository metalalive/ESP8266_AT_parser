...
espRes_t eESPpktBufCopy(espPbuf_t *des_p, void *src_p, size_t len) {
    ...
    if (cpy_len != len) {
        ...
        // some of received IPD data are missing because of insufficient payload
        // space.
        response = espERR;
    }
    return response;
...
void vESPpktBufItemDelete(espPbuf_t *buff) { // no need to dealloc espPbuf_t.payload since it
                                             // was allocated together with espPbuf_t

...
espRes_t eESPgetCurrATversion(void);
...
espRes_t eESPenterDeepSleep(
    uint32_t sleep_ms, const espApiCmdCbFn api_cb, void *const api_cb_arg, const uint32_t blocking
);
espRes_t
eESPsetBaud(uint32_t baud, const espApiCmdCbFn cb, void *const cb_arg, const uint8_t blocking);
...
// the entry point to process the received data from ESP device and analyze the
// result of the previously sent AT command.
espRes_t eESPprocessRecvDataFromDev(const void *data, size_t len);

// once user thread calls API functions,
// * one thread, created by this AT software, dedicates to collecting the
// requests generated from these API functions,
...
// service routine that should be called in UART Rx / DMA interrupt in order to
// transmit pieces of received response  data string (if very large) to
// response-handling thread above (vESPthreadATrespHandler)
...
// helper function to set up everything essential for starting / stopping a
// server
espRes_t
...
espRes_t eESPconnClientClose(
    espConn_t *conn_in, espApiCmdCbFn cb, void *const cb_arg, const uint32_t blocking
);

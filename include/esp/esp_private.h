...
/* TCP/IP related commands */
    ...
    ESP_CMD_TCPIP_CIPMODE,    /*!< Transmission mode, either transparent or normal
                                 one */
    ESP_CMD_TCPIP_CIPSTO,     /*!< Sets connection timeout */
#if (ESP_CFG_CONN_MANUAL_TCP_RECV != 0)
    ESP_CMD_TCPIP_CIPRECVMODE, /*!< Sets mode for TCP data receive (manual or
                                  automatic) */
...
#endif                         /* ESP_SNT  */
    ESP_CMD_TCPIP_CIPDINFO,    /*!< Configure what data are received on +IPD
                                  statement */
} espCmd_t;
...
typedef struct espMsg {
    ...
    /*  Note: for any API function executing extended AT-command sequence
        ...
        pros: keep non-blocking feature, sequence of non-blocking AT-commands
       can be safely executed, this also improves ease of code maintenance.
        cons: one msg structure will NOT be shared among these extends AT
    ...
    union {
        struct {
            uint32_t delay; /*!< Delay in units of milliseconds before executing
                               first RESET command */
        } reset;            /*!< Reset device */
        struct {
            uint32_t ms;
        ...
        struct {
            espFnEn_t ext_info_netconn; // do we enable extra information to describe
                                        // currently processing network connection ?
            espFnEn_t ext_info_ipd;     // do we enable extra information to
                                        // describe currently processing IPD data ?
        } sysargs;
        struct {
        ...
        struct {
            ...
            uint8_t         def; /*!< Value indicates to connect as current only or as
                                    default */
        ...
        struct {
            ...
            uint16_t   *num_ap_found; /*!< Pointer to output variable holding
        ...
        struct {
            ...
            size_t   *staf; /*!< Pointer to output variable holding number of
                               access points found */
        ...
        struct {
            ...
            uint8_t   def; /*!< Value for receiving default or current settings */
        } sta_ap_getmac;   /*!< Message for reading station or access point MAC
                              address */
        struct {
        ...
        struct {
            ...
            uint8_t         def; /*!< Value for receiving default or current settings */
        } sta_ap_setmac;         /*!< Message for setting station or access point MAC
                                    address */
        struct {
            espIp_t  *sta_ip;
            espMac_t *sta_mac;
        ...
        struct {
            ...
            uint16_t       d_size;      /*!< size of data (bytes) ready to transfer,
                                           shouldn't be greater than 2048 */
            const espIp_t *remote_ip;   /*!< Remote IP address for UDP connection */
            espPort_t      remote_port; /*!< Remote port address for UDP connection */
            uint8_t        val_id;      /*!< Connection current validation ID when command
                                           was sent to queue */
        ...
        struct {
            espEvtCbFn cb;      // callback functions for server, especially useful
                                // when the server receives IPD data,
            uint8_t   en;       /*!< Enable/Disable server status */
            espPort_t port;     /*!< Server port number */
            uint16_t  max_conn; /*!< Maximal number of connections available for
                                   server */
...
// collect attribute of a network device (AP or station) like IP, MAC address,
// and gateway address.
typedef struct {
...
struct espGlbl_s {
    ...
    espSysMbox_t mbox_cmd_req;            /*!< buffer AT-command requests sent from API
                                             function calls */
    espSysMbox_t mbox_cmd_resp;           /*!< buffer AT-command responses received from
                                             ESP device */
    ...
    espEvtCbFnLstItem_t *evtCbLstHead;    /*!< starting point to the linked list of
    ...
    union {
        struct {
            espMultiConn_t mux_conn; // indicate whether current setting is
                                     // multiple connection.
            uint8_t initialized : 1; /*!< Flag indicating ESP library is
                                        initialized */
            uint8_t dev_present : 1; /*!< Flag indicating if physical device is

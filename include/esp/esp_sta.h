...
espRes_t    eESPstaAutojoin(uint8_t en, const espApiCmdCbFn cb, void* const cb_arg, const uint32_t blocking);
espRes_t    eESPstaGetIP( espIp_t* ip, espIp_t* gw, espIp_t* nm, uint8_t saveDef, 
                          const espApiCmdCbFn cb, void* const cb_arg, const uint32_t blocking );
espRes_t    eESPstaSetIP( const espIp_t* ip, const espIp_t* gw, const espIp_t* nm, uint8_t saveDef, 
                          const espApiCmdCbFn cb, void* const cb_arg, const uint32_t blocking );
espRes_t    eESPstaGetMAC(espMac_t* mac, uint8_t def, const espApiCmdCbFn cb, void* const cb_arg, const uint32_t blocking);
espRes_t    eESPstaSetMAC(const espMac_t* mac, uint8_t def, const espApiCmdCbFn cb, void* const cb_arg, const uint32_t blocking);

espRes_t    eESPstaCopyIP(espIp_t* ip, espIp_t* gw, espIp_t* nm);
espRes_t    eESPstaGetAPinfo( espStaInfoAP_t* info, const espApiCmdCbFn cb, void* const cb_arg, const uint32_t blocking );

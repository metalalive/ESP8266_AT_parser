...
// result / status transferred among core functions of this library
typedef enum {
    espOK = 0,              // Function succeeded 
    espOKIGNOREMORE,        // Function succedded, but ignore sending more data. This result is possible on connection data receive callback 
    espOKNOCMDREQ,          // few API functions can get their job done wihtout generating AT command request.
                            // such the API calls can return this state.
    ...
    espINPROG,              // Operation is in progress, used in both AT-command mode and IPD receiving mode. 
...
typedef enum {
    ...
    ESP_CONN_TYPE_SSL,       // connection type is SSL, should be deprecated since it was proven insecure.
} espConnType_t;
...
// event structure that contains necessary information and passed to callback function
typedef struct {
    espEvtType_t   type;              /* Callback type */
    union {
        struct {
            uint8_t forced;           /* Set to `1` if reset is forced by user */
        } reset_detected;             /* Reset occurred. Use with ESP_EVT_RESET_DETECTED event */
        struct {
        ...
        } connDataRecv;               /* Network data received. Use with ESP_EVT_CONN_RECV event */
        struct {
        ...
        } connError;                  /* Client connection start error. Use with ESP_EVT_CONN_ERROR event */
        struct {
        ...
        } connActiveClosed;           /* Process active and closed statuses at the same time. Use with ESP_EVT_CONN_ACTIVE or ESP_EVT_CONN_CLOSED events */
        struct {
            struct espConn *conn;          /* Set connection pointer */
        } connPoll;                   /* Polling active connection to check for timeouts. Use with \ref ESP_EVT_CONN_POLL event */
        struct {
        ...
        } staListAP;                       /* Station list access points. Use with ESP_EVT_STA_LIST_AP event */
        struct {
            espRes_t res;                  /* Result of command */
        } staJoinAP;                       /* Join to an access point. Use with ESP_EVT_STA_JOIN_AP event */
        struct {
            espStaInfoAP_t* info;          /* AP info on which current station is connected to */
            espRes_t res;                  /* Result of command */
        } staInfoAP;                       /* Current AP informations (only for current connected AP ?). Use with ESP_EVT_STA_INFO_AP event */
#endif 
#if ESP_CFG_MODE_ACCESS_POINT || __DOXYGEN__
        struct {
            espMac_t* mac;                      /* Station MAC address */
        } apConnDisconnSta;                     /* A new station connected or disconnected to ESP's access point. 
                                                   Use with ESP_EVT_AP_CONNECTED_STA or \ref ESP_EVT_AP_DISCONNECTED_STA events */
        struct {
            espMac_t* mac;                      /* Station MAC address */
            espIp_t*  ip;                       /* Station IP address */
        } staIPfromAP;                          /* Station got IP address from ESP's access point. Use with \ref ESP_EVT_AP_IP_STA event */
#endif 
#if ESP_CFG_DNS || __DOXYGEN__
        struct {
            espRes_t res;                       /* Result of command */
        ...
        } dnsHostbyname;                       /* DNS domain service finished. Use with \ref ESP_EVT_DNS_HOSTBYNAME event */
#endif 
#if ESP_CFG_PING || __DOXYGEN__
        struct {
            espRes_t res;                       /* Result of command */
            const char* host;                   /* Host name for ping */
            uint32_t *resptime;                 /* Time required for ping. Valid only if operation succedded */
        } ping;                                 /* Ping finished. Use with \ref ESP_EVT_PING event */
#endif 
    } body;                                     /* main body of event structure ( defined by union { ... }, the content depends on event type) */
} espEvt_t;
...
typedef struct {
    ...
    struct {
        ...
        // [TODO] determine if we really need to keep the following handling objects to send characters
        // void      *handler;                     // the pointer to the object that sends / receives (AT command) string characters 
        //                                         // to/from ESP hardware device, this is specified according to target hardware platform.
        // void      *dbg_printer;                 // the pointer to the object that sends / receives characters for debugging purpose.
        //                                         // This is specified according to target hardware platform.

...
// address. The following value must be power of 2.
#ifndef ESP_CFG_MEM_ALIGNMENT
#define ESP_CFG_MEM_ALIGNMENT  4
#endif // end of ESP_CFG_MEM_ALIGNMENT


// maximum number of connections this AT-command library can support on ESP device,
// for many of ESP8266 devices (e.g. ESP-01) , the maximum number of supported TCP
// connections is 5, so we set the default value to following macro.
...
// enable / disable station mode in the AT library. For any ESP device in station mode,
// it can connect to other access point(s) found.
#ifndef ESP_CFG_MODE_STATION 
#define ESP_CFG_MODE_STATION  1
#endif // end of ESP_CFG_MODE_STATION


// enable / disable access-point mode in the AT library. For any ESP device acting as access point,
// it can accept connections from  other station(s).
...
// When this function is enabled, it will automatically clear the current setting stored 
// on the ESP device, and load factory settings to the device.
// [Note] 
...
// Default delay time (in milliseconds) before sending first AT command on reset sequence
...
// Maximum number of items stored in a message box for AT-command-request-handling thread,
// [Note] Message queue is used for storing address to AT-command data.
#ifndef ESP_CFG_AT_CMD_REQ_MBOX_SIZE
#define ESP_CFG_AT_CMD_REQ_MBOX_SIZE   16
#endif

// Maximum number of items stored in a message queue for AT-command-response-handling thread,
...
//  queue length for apccepting number of new client connections used in netconn server
//  , it represents number of maximal clients waiting in accept queue of server connection
#ifndef ESP_CFG_NETCONN_ACCEPT_Q_LEN
#define ESP_CFG_NETCONN_ACCEPT_Q_LEN   ESP_CFG_MAX_CONNS 
#endif

//  queue length for incoming data (received on Rx of ESP device), packed within pbuf structure.
...
// Memory copy function declaration, users can specify other memory copy function in case
// they implement their own function for specific hardware architecture.
// (e.g. make use of hardware acceleration to speed up memory copy operations)
#ifndef ESP_MEMCPY

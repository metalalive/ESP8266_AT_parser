...
// esp_sys provides functions from underlying operating system like thread, semaphore, mutex, delay function ... etc.
#ifdef __cplusplus
extern "C" {
#endif
...
// * --- low-level hardware-specific functions for resetting & sending AT command to ESP device 
...
// get current time, the format depends on system implementation, (it can be ticks, ms, us... etc)
uint32_t   uESPsysCurrTime( void );

// the lock for atomic access (among multiple threads) happened to the core function of ESP AT library
...
espRes_t   eESPsysThreadCreate( espSysThread_t* t, const char* name, 
                                espSysThreFunc  thread_fn, void* const arg,
                                size_t stack_size, espSysThreadPrio_t prio, 

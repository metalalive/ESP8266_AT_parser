...
espRes_t    eESPinitATcmd( espMsg_t* msg )
{
    ...
    switch( GET_CURR_CMD(msg) ) 
    {
	    ...
	    AT_APPEND_STR( &cmd_str_p, (msg->body.wifi_mode.def==ESP_SETVALUE_SAVE ? (const uint8_t *)&"+CWMODE_DEF=" : (const uint8_t *)&"+CWMODE_CUR=") , 12 ); 
	    AT_APPEND_CHR( &cmd_str_p, ESP_NUMTOCHAR(msg->body.wifi_mode.mode) ); 
            break;            
#if (ESP_CFG_MODE_STATION != 0)
        case ESP_CMD_WIFI_CWJAP                  :
	    AT_APPEND_STR( &cmd_str_p, (msg->body.sta_join.def==ESP_SETVALUE_SAVE ? (const uint8_t *)&"+CWJAP_DEF=" : (const uint8_t *)&"+CWJAP_CUR=") , 11 );
	    AT_APPEND_CHR( &cmd_str_p, ESP_ASCII_DOUBLE_QUOTE );
	    AT_APPEND_STR( &cmd_str_p, (const uint8_t *)&msg->body.sta_join.name[0], msg->body.sta_join.name_len );
	    AT_APPEND_CHR( &cmd_str_p, ESP_ASCII_DOUBLE_QUOTE );
	    AT_APPEND_CHR( &cmd_str_p, ESP_ASCII_COMMA );
	    AT_APPEND_CHR( &cmd_str_p, ESP_ASCII_DOUBLE_QUOTE );
	    AT_APPEND_STR( &cmd_str_p, (const uint8_t *)&msg->body.sta_join.pass[0], msg->body.sta_join.pass_len );
	    AT_APPEND_CHR( &cmd_str_p, ESP_ASCII_DOUBLE_QUOTE );
	    ...
	    AT_APPEND_STR( &cmd_str_p, (msg->body.sta_ap_setip.def==ESP_SETVALUE_SAVE ? (const uint8_t *)&"+CIPSTA_DEF=" : (const uint8_t *)&"+CIPSTA_CUR=") , 12 ); 
	    ...
	    AT_APPEND_STR( &cmd_str_p, (msg->body.sta_ap_setip.def==ESP_SETVALUE_SAVE ? (const uint8_t *)&"+CIPAP_DEF=" : (const uint8_t *)&"+CIPAP_CUR=") , 11 ); 
    ...
    /* TCP/IP related commands */ 
	    ...
	    AT_APPEND_CHR( &cmd_str_p, ESP_ASCII_DOUBLE_QUOTE );
            ...
            AT_APPEND_STR( &cmd_str_p, (msg->body.conn_start.type == ESP_CONN_TYPE_TCP ? (const uint8_t *)&"TCP": (const uint8_t *)&"UDP"), 3 ); 
	    AT_APPEND_CHR( &cmd_str_p, ESP_ASCII_DOUBLE_QUOTE );
	    AT_APPEND_CHR( &cmd_str_p, ESP_ASCII_COMMA ); 
	    AT_APPEND_CHR( &cmd_str_p, ESP_ASCII_DOUBLE_QUOTE );
            AT_APPEND_STR( &cmd_str_p, (const uint8_t *)&msg->body.conn_start.host[0], msg->body.conn_start.host_len ); 
	    AT_APPEND_CHR( &cmd_str_p, ESP_ASCII_DOUBLE_QUOTE );
	    AT_APPEND_CHR( &cmd_str_p, ESP_ASCII_COMMA ); 
	    ...
	    AT_APPEND_CHR( &cmd_str_p, ESP_ASCII_DOUBLE_QUOTE );
	    AT_APPEND_STR( &cmd_str_p, (const uint8_t *)&msg->body.tcpip_ping.host[0], msg->body.tcpip_ping.host_len );
	    AT_APPEND_CHR( &cmd_str_p, ESP_ASCII_DOUBLE_QUOTE );
            break;     
#endif /* ESP_CFG_PING  */
